...
traj_in = f'spec_prod.xtc'
gro = 'VPG_solv_ions.gro'

npy_out = f'spec_traj_{feature_name}.npy'
...
# Add N-H pairs (protein atoms only)
...
# Add O-H pairs (protein atoms only)
for i in range(len(oxygen_atoms)):
    for j in range(len(hydrogen_atoms)):
        # Ensure the oxygen and hydrogen are from residues that are at least one residue apart
